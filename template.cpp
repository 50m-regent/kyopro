...
template<class T>
int lb(vec<T>& x,T n){return lower_bound(all(x) , n) - x.begin();}
template<class T>
int ub(vec<T>& x,T n){return upper_bound(all(x) , n) - x.begin();}
// ループ
...
template <class T>
void print(vec<vec<T>> &vv) {each(v, vv){print(v);}}
// デバッグ出力
...
modint mcomb(int n, int r){return comb(n + r - 1, n - 1);} // r個をn部屋に分ける
// UnionFind
struct UF{
    vector<int> t;
